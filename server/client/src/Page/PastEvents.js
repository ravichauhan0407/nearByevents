...
import {fetchPastEvent} from '../helper/FetchEvent'
import SearchBox from '../components/SearchBox'
import { localDate } from '../helper/LocalDate'

const PastEvents = () => {
  const [pastEventList,setLiveEventList]=useState([]);
  const [cityFilter,setCityFilter]=useState('')
  const [currentPage, setCurrentPage] = useState(1);

 useEffect(()=>
 {
    fetchPastEvent({city:cityFilter}).then((data)=>
    {
 ...
 const currentEvents = pastEventList.slice(indexOfFirstEvent,indexOfLastEvent);
 ...
 const paginate = pageNumber => setCurrentPage(pageNumber);
  ...
  return (
   ...
   {currentEvents.length>0&&<div class="row">
    {currentEvents.map((item) => {
        ...
        return (
            <div  className='col-xs-4 col-sm-6 col-md-4' key={item._id}><div className="card text-left" style={{marginBottom:"30px"}}>
  ...
  <Pagination
        eventsPerPage={12}
        totalEvents={pastEventList.length}
        paginate={paginate}
      />
