...
import { fetchLiveEvent} from '../helper/FetchEvent'
...
const LiveEvents = () => {
 ...
 useEffect(()=>
 {
    fetchLiveEvent({city:cityFilter}).then((data)=>
    {
 ...
 const currentEvents = liveEventList.slice(0,1);
 ...
 const paginate = pageNumber => setCurrentPage(pageNumber);
  ...
  <Pagination
        eventsPerPage={12}
        totalEvents={liveEventList.length}
        paginate={paginate}
      />
  </div>
  )
